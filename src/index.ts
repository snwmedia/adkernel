...
class Common {
    ...
    static async getAllBundles(token: string, timeRange: string, startFrom: number, bundlesReport: any[], limit?: number): Promise<any[]> {
...
export class RTB {
    public static async getAllAppBundles(timeRange: string, limit?: number) {
        let token = await Common.getToken();
        let bundlesReport: any[] = await Common.getAllBundles(token, timeRange, 0, [], limit);
        return bundlesReport;
    }
}